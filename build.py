...
def should_update_dll(config: Config, version: str) -> bool:
    if config.is_windows():
        ...
        if not Path("src/native/linux/x64/libautd3capi_link_soem.so").is_file() or not Path("tests/libautd3capi_link_soem.so").is_file():
            return True
...
def copy_dll(config: Config) -> None:
    ...
    download_and_extract(f"{base_url}/autd3-link-soem-v{version}-win-x64-shared.zip", "src/native/windows/x64", "tests")
    download_and_extract(f"{base_url}/autd3-link-soem-v{version}-win-aarch64-shared.zip", "src/native/windows/arm")
    download_and_extract(f"{base_url}/autd3-link-soem-v{version}-macos-aarch64-shared.tar.gz", "src/native/osx/aarch64", "tests")
    download_and_extract(f"{base_url}/autd3-link-soem-v{version}-linux-x64-shared.tar.gz", "src/native/linux/x64", "tests")
...
def rm_tmp_source() -> None:
    _ = subprocess.run(
        ["dotnet", "nuget", "remove", "source", "autd3sharp_link_soem_local"],
        check=False,
        capture_output=True,
    )
    _ = subprocess.run(
...
def should_update_dll_unity(config: Config, version: str) -> bool:
    if config.is_windows():
        ...
        if not Path("unity/Assets/Plugins/x86_64/libautd3capi_link_soem.so").is_file():
            return True
...
def copy_dll_unity(config: Config) -> None:
    ...
    if not should_update_dll_unity():
        return
    base_url = f"https://github.com/shinolab/autd3-capi-link-soem/releases/download/v{version}"
    download_and_extract(f"{base_url}/autd3-link-soem-v{version}-win-x64-unity.zip", "unity/Assets/Plugins/x86_64")
    download_and_extract(f"{base_url}/autd3-link-soem-v{version}-win-aarch64-unity.zip", "unity/Assets/Plugins/ARM64")
    download_and_extract(f"{base_url}/autd3-link-soem-v{version}-macos-aarch64-unity.tar.gz", "unity/Assets/Plugins/aarch64")
    download_and_extract(f"{base_url}/autd3-link-soem-v{version}-linux-x64-unity.tar.gz", "unity/Assets/Plugins/x86_64")
...
def copy_dll_unity_dynamic_freq(config: Config) -> None:
    ...
    if not should_update_dll_unity():
        return
    base_url = f"https://github.com/shinolab/autd3-capi-link-soem/releases/download/v{version}"
    download_and_extract(f"{base_url}/autd3-link-soem-v{version}-win-x64-unity-dynamic_freq.zip", "unity-dynamic_freq/Assets/Plugins/x86_64")
    download_and_extract(f"{base_url}/autd3-link-soem-v{version}-win-aarch64-unity-dynamic_freq.zip", "unity-dynamic_freq/Assets/Plugins/ARM64")
    download_and_extract(
        f"{base_url}/autd3-link-soem-v{version}-macos-aarch64-unity-dynamic_freq.tar.gz", "unity-dynamic_freq/Assets/Plugins/aarch64"
    )
    download_and_extract(f"{base_url}/autd3-link-soem-v{version}-linux-x64-unity-dynamic_freq.tar.gz", "unity-dynamic_freq/Assets/Plugins/x86_64")
...
def unity_build(args) -> None:  # noqa: ANN001
    ...
    def _build(parent) -> None:
        ...
        remove(f"{parent}/Assets/Scripts/AUTD3Sharp.Link.SOEM.csproj")
        remove(f"{parent}/Assets/Scripts/AUTD3Sharp.Link.SOEM.nuspec")
